...
function RegisterGarment({ route, navigation }) {
  ...
  const onRegister = (data) => {
    ...
    if (imageUrl != null && model && data.price > data.advance) {
      onRegisterGarment(formData, userAuth.token)
        .then(
          async response => {
            setEditable(true)
            if (response.data) {
    ...
    } else if (data.price < data.advance) {
      setEditable(true)
...
export default RegisterGarment